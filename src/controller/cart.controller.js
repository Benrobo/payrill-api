...
class CartControler {
    ...
    async importCart(res, cartId){
        ...
        try {
            ...
            if(result.length != 0){
                ...
                sendResponse(res, 400, false, "Ecart Not Found", {});
            }
        } catch (error) {
            sendResponse(res, 400, false, "Error Importing Ecart", error);
        }
    }

    async approveCart(res, payload) {
        const {id} = res.user;
        const {cartId,pin} = payload
        ...
        db.query({
            ...
            if (toHash(String(pin)) === results[0].pin) {
                try {
                   let result = await query("SELECT * FROM ecart WHERE id = ?",[cartId]);
                   if(result.length != 0){
                        const ecart = result[0];
                        if(ecart.store_id != id){
                            return sendResponse(res, 400, false, "Access to Ecart Denied", {});
                        }
                        if(ecart.paid !== "true"){
                            return sendResponse(res, 400, false, "Ecart Not Paid For!", {});
                        }
                        if(ecart.confirmed === "true"){
                            return sendResponse(res, 400, false, "Ecart Already Confirmed", {});
                        }

                        // Approve Ecart
                        await query("UPDATE ecart SET confirmed = ? WHERE id = ?",["true",cartId]);

                        sendResponse(res, 200, true, "Ecart Confirmed", {});
                   }else{
                        sendResponse(res, 400, false, "Ecart Not Found", {});
                   }
                } catch (error) {
                    sendResponse(res, 400, false, "An Error Occured", error);
                }
            } else {
    ...
    async getEcartForOrg(res, ecartId) {
        try {
            ...
            console.log(e)
            return sendResponse(res, 500, false, "Something went wrong fetching items.", {});
        }
    }
    ...
    async payForCart(res, payload) {
        ...
        db.query({
            ...
            if (ecart.paid === "false" && ecart.confirmed === "false") {
                ...
                db.query({
                    ...
                    if (toHash(String(pin)) === results[0].pin) {
                        db.query({
                            ...
                            // console.log(payload);
                            try {
                                let result = await Fetch("POST", "/v1/account/transfer", payload);
                                ...
                                console.log(e);
                                return sendResponse(res, 400, false, "An Error Occured", e.body);
                            }
                        });
                    } else {
                        sendResponse(res, 400, false, "Incorrect Pin", {});
                ...
                return sendResponse(res, 400, false, "Ecart Already Paid For!", {});
            }
        });
    }
    ...
    async refundCart(res, payload) {
        ...
        db.query({
            ...
            if (ecart.paid === "true" && ecart.confirmed == "false") {
                    ...
                    try {
                        ...
                        if (tranStatus) {
                            db.query({
                                sql: "UPDATE ecart SET paid = ?, amount = ? WHERE (user_id = ? AND store_id = ? AND id = ?)",
                                timeout: 40000,
                                values: ["false", 0, id, storeId, ecartId, ],
                            }, async function(error, results) {
                                return sendResponse(res, 200, true, "Refunded", result.body.data);
                            });
                        }
                    } catch (e) {
                        console.log(e);
                ...
                return sendResponse(res, 400, false, "Can Not Refund Ecart", {});
            }
        });
    }
    ...
    async createEcart(res, payload) {
        ...
        db.query({
            ...
            if (error) {
                console.log(error);
                return sendResponse(res, 400, true, "Error Creating Ecart", {});
            }
    ...
    async addToEcart(res, payload) {
        ...
        db.query({
            ...
            if (ecart.paid === "true") {
                return sendResponse(res, 400, false, "You can not add items to this Cart!", {});
            }
            ...
            db.query({
                ...
                if (results.length != 0) {
                    ...
                    if (storeId !== item.store_id) {
                        return sendResponse(res, 400, false, "You can not add this item to this cart!", {});
                    }
                    ...
                    db.query({
                        ...
                        if (results.length == 0) {
                            ...
                            db.query({
                                ...
                                timeout: 40000,
                                values: [newCheckoutItem_qty, cartId, itemId, ],
                            }, function(error, results) {
                                // Update Item Quantity in store
                                db.query({
                                    sql: "UPDATE store_items SET item_quantity = ? WHERE (id = ?)",
